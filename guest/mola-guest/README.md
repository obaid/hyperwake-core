...
make test
make build      # bin/mola-guest-linux-{arm64,amd64}
```
...
the rest of an injected payload. `MOLA_SMBIOS_PATH` overrides the DMI path
...
|---|---|---|
| `MOLA_ENDPOINT` | — | control plane base URL. Required unless supplied via SMBIOS |
| `MOLA_REGISTRATION_TOKEN` | — | single-use; only needed on first boot |
| `MOLA_COMPUTER_ID` | from SMBIOS | the control plane's id for this machine |
| `MOLA_MACHINE_ID` | generated | per-machine identity |
| `MOLA_SMBIOS_PATH` | `/sys/class/dmi/id/product_serial` | override for testing |
| `MOLA_STATE_DIR` | `/var/lib/mola` | must be on the persistent disk |
| `MOLA_HEARTBEAT_INTERVAL` | `15` | seconds; the control plane may override |
| `MOLA_HTTP_TIMEOUT` | `15s` | bounds every call home |
| `MOLA_SHUTDOWN_COMMAND` | `/sbin/shutdown -h now` | how to stop the guest |
...
`MOLA_STATE_DIR` at a path on the persistent volume.
...
`packaging/mola-guest.service` is the systemd unit for the real image. The
