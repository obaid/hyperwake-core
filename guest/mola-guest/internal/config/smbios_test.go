...
func TestTolerantOfSurroundingWhitespace(t *testing.T) {
	...
	if got == nil || got.Endpoint != "https://mola.ai" {
...
func TestAcceptsBase64Variants(t *testing.T) {
	payload, err := json.Marshal(Bootstrap{Endpoint: "https://mola.ai", RegistrationToken: "t"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadFallsBackToSMBIOSWhenEnvIsEmpty(t *testing.T) {
	...
	t.Setenv("MOLA_REGISTRATION_TOKEN", "")
	t.Setenv("MOLA_COMPUTER_ID", "")
	t.Setenv("MOLA_SMBIOS_PATH", path)
...
func TestEnvironmentWinsOverSMBIOS(t *testing.T) {
	...
	t.Setenv("MOLA_REGISTRATION_TOKEN", "env-token")
	t.Setenv("MOLA_COMPUTER_ID", "env-computer")
	t.Setenv("MOLA_SMBIOS_PATH", path)
...
func TestEnvironmentAndSMBIOSMergePerField(t *testing.T) {
	...
	t.Setenv("MOLA_ENDPOINT", "http://override.test")
	t.Setenv("MOLA_REGISTRATION_TOKEN", "")
	t.Setenv("MOLA_COMPUTER_ID", "")
	t.Setenv("MOLA_SMBIOS_PATH", path)
...
func TestCorruptSMBIOSIsIgnoredWhenEnvIsSufficient(t *testing.T) {
	...
	t.Setenv("MOLA_REGISTRATION_TOKEN", "env-token")
	t.Setenv("MOLA_COMPUTER_ID", "")
	t.Setenv("MOLA_SMBIOS_PATH", writeSerial(t, "!!!garbage!!!"))
