...
type Config struct {
	// Endpoint is the control plane base URL, e.g. http://hyperwake.test.
	Endpoint string
...
const (
	...
	defaultStateDir          = "/var/lib/hyperwake"
...
func Load() (*Config, error) {
	c := &Config{
		Endpoint:           strings.TrimRight(os.Getenv("HYPERWAKE_ENDPOINT"), "/"),
		RegistrationToken:  os.Getenv("HYPERWAKE_REGISTRATION_TOKEN"),
		MachineID:          os.Getenv("HYPERWAKE_MACHINE_ID"),
		ComputerID:         os.Getenv("HYPERWAKE_COMPUTER_ID"),
		StateDir:           envOr("HYPERWAKE_STATE_DIR", defaultStateDir),
		AuthorizedKeysPath: envOr("HYPERWAKE_AUTHORIZED_KEYS_PATH", defaultAuthorizedKeys),
		HeartbeatInterval:  envDuration("HYPERWAKE_HEARTBEAT_INTERVAL", defaultHeartbeatInterval),
		HTTPTimeout:        envDuration("HYPERWAKE_HTTP_TIMEOUT", defaultHTTPTimeout),
		Version:            Version,
		OS:                 detectOS(),
		Source:             "environment",
	}

	if cmd := os.Getenv("HYPERWAKE_SHUTDOWN_COMMAND"); cmd != "" {
		c.ShutdownCommand = strings.Fields(cmd)
	} else {
		c.ShutdownCommand = []string{"/sbin/shutdown", "-h", "now"}
	}

	if err := c.applyBootstrap(envOr("HYPERWAKE_SMBIOS_PATH", SMBIOSPath)); err != nil {
		return nil, err
	}

	if c.Endpoint == "" {
		return nil, errors.New("no endpoint: set HYPERWAKE_ENDPOINT or inject an SMBIOS bootstrap payload")
	}
